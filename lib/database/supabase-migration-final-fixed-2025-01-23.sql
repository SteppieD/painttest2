-- Supabase Migration Final Fixed for PaintQuoteApp
-- Date: 2025-01-23
...
-- Add unique constraint to company_id if it doesn't exist
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'company_preferences_company_id_key') THEN
        ALTER TABLE public.company_preferences ADD CONSTRAINT company_preferences_company_id_key UNIQUE (company_id);
    END IF;
END $$;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'company_branding_company_id_key') THEN
        ALTER TABLE public.company_branding ADD CONSTRAINT company_branding_company_id_key UNIQUE (company_id);
    END IF;
END $$;
...
-- Insert default preferences for companies that don't have them (without ON CONFLICT)
...
-- ============================================
-- PART 5: Create or update triggers
-- ============================================

-- Only create triggers if they don't exist
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_company_preferences_updated_at') THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_company_paint_products_updated_at') THEN
...
FROM pg_constraint 
WHERE conname IN ('company_preferences_company_id_key', 'company_branding_company_id_key');
